...
<body>
    ...
    <div class="container max-w-full mx-auto pt-4" style="width:900px">
        ...
        <form method="POST" action="/posts/{{ $post->id }}" class="mt-5 w-full max-w-sm">
            @method('PUT')
            @csrf
            ...
            <input
                ...
                id="inline-title" type="text" name="title" value="{{ $post->title }}">
            ...
            <textarea
                class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500"
                id="inline-content" name="content">{{ $post->content }} </textarea>
